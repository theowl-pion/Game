...
import {
  ...
  CalendarIcon,
  GiftIcon,
} from "@heroicons/react/outline"; // Assuming heroicons are installed
  ...
  const fetchGains = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:4000/api/user/my-gains",
        {
      ...
      if (err.response?.status === 401 || err.response?.status === 403) {
        ...
        setErrorGains(
          err.message || "An error occurred while fetching winnings."
        );
      }
    } finally {
      setIsLoadingGains(false);
      ...
      {userData && !isLoadingProfile && !errorProfile && (
        <div className="grid grid-cols-1 gap-8">
          ...
          <div className="mt-8 bg-white p-6 rounded-lg shadow-md border border-gray-200">
            ...
            {isLoadingGains && (
              <p className="text-gray-600">Chargement des gains...</p>
            )}
            ...
            {errorGains && (
              ...
              >
                <span className="block sm:inline">
                  Erreur lors du chargement des gains: {errorGains}
                </span>
              </div>
            )}
            ...
            {!isLoadingGains &&
              ...
              (gainsData.length > 0 ? (
                ...
                <p className="text-gray-600">
                  Vous n'avez pas encore enregistré de gains.
                </p>
              ))}
          </div>
        </div>
